...
def read_config_values():
    try:
        ...
        cursor.execute(
            f"SELECT key_name, value FROM config_values WHERE server_name='{server_name}'"
        )
...
def start_rust():
    ...
    rust_cmd = [
        ...
        "-silent-crashes",
        "-logfile",
        os.environ["SERVER_LOGS"],
        "+server.ip",
        "0.0.0.0",
        "+server.port",
        os.environ["RUST_SERVER_PORT"],
        "+rcon.ip",
        "0.0.0.0",
        "+rcon.port",
        os.environ["RUST_RCON_PORT"],
        "+app.publicip",
        os.environ["SERVER_IP"],
        "+app.port",
        os.environ["RUST_APP_PORT"],
        "+rcon.web",
        "1",
        "+rcon.password",
        os.environ["RUST_RCON_ADMIN"],
        "+server.gamemode",
        "vanilla",
        "+server.identity",
        os.environ["RUST_IDENTITY"],
        "+server.logoimage",
        os.environ["RUST_AVATAR"],
    ]
